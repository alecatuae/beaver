/* eslint-disable unicorn/prefer-module */ /* eslint-disable no-magic-numbers */ /* eslint-disable no-nested-ternary */ import { mkdir, writeFile } from 'node:fs';
...
generatorHandler({
    ...
    onGenerate: async (options) => {
        ...
        if (outputLocation === defaultOutput) {
            await generateOutput(options.dmmf, prismaTypes, prismaLocation.startsWith("@") ? prismaLocation : posix.join(prismaLocation, "index.js"), join(outputLocation, "../esm/generated.ts"));
        }
    }
});
async function generateOutput(dmmf, prismaTypes, prismaLocation, outputLocation) {
    const importStatement = ts.factory.createImportDeclaration(...modifiersArg, [], ts.factory.createImportClause(true, undefined, ts.factory.createNamedImports([
