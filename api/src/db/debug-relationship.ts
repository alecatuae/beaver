...
async function debugRelationship() {
  ...
  if (!relationshipId) {
    console.error('Uso: npx ts-node src/db/debug-relationship.ts <ID_DO_RELACIONAMENTO>');
    process.exit(1);
  }
  ...
  try {
    ...
    try {
      ...
      if (result4.records.length > 0) {
        ...
        const targetIdPrefix = relationshipId.substring(0, Math.min(10, relationshipId.length));
